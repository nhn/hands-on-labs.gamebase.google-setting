...
* Assets/NHN FORWARD/GamebaseSample/Script/Authentication/GamebaseAuthenticateSample.cs 파일의 OnLogin() 함수를 다음과 같이 수정
...
.. code-block:: C#
    ...
    public void OnLogin()
    {
        Gamebase.Login(GamebaseAuthProvider.GOOGLE, (authToken, error) =>
        {
            ...
            {
                statusText.text = string.Format("Google login succeeded. User id is {0}", authToken.member.userId);
            }
            else
            {
                statusText.text = string.Format("Google login failed. Error is {0}", error.ToString());
            }
        });
    }
